...
#ps aux | grep $(whoami) | grep -v "sshd\|bash\|zsh\|grep" | awk '{print $2}' | xargs -r kill -9 > /dev/null 2>&1
pkill -kill -u $(whoami)
...
download_singbox() {
  ...
  if [ "$ARCH" == "arm" ] || [ "$ARCH" == "arm64" ] || [ "$ARCH" == "aarch64" ]; then
      ...
      FILE_INFO=("https://github.com/usc/build-sing-box/releases/latest/download/sing-box-freebsd-amd64 web" "https://github.com/eooce/test/releases/download/freebsd/server bot" "https://github.com/eooce/test/releases/download/freebsd/npm npm")
  else
...
rm -f "$(basename ${FILE_MAP[npm]})" "$(basename ${FILE_MAP[bot]})"
}
...
get_ip() {
  ...
  if [ -z "$ip" ]; then
    ip=$( [[ "$HOSTNAME" =~ ^s([0-9]|[1-2][0-9]|30)\.serv00\.com$ ]] && dig +short $HOSTNAME | grep -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}' | head -n 1)
  fi
  echo "$ip"
}
